...
varspec = [
  ...
  [ 'distro',                 'poky-swi',    'Distribution label, defaulting '
                                             'to poky-swi'                   ],
  [ 'machine-type',           'swi-mdm9x28', 'Sierra machine type, defaulting '
                                             'to swi-mdm9x28'                ],
  ...
  [ 'kernel-provider',        '',            'Name of kernel provider '
  ...
  [ 'ima-config-file',        '',            'Path to IMA config file. '
                                             'This is used as-is, '
                                             'not canonicalized.'            ],
  ...
  [ 'enable-prop-bin',        False,         'Enable proprietary Qualcomm '
                                             'binary packages.'              ],
  [ 'enable-prop-src',        False,         'Enable proprietary Qualcomm '
                                             'source packages.'              ],
  [ 'apps-proc-dir',          cpth(),        'Path to Qualcomm firmware '
                                             'tree, usually an "apps_proc" '
                                             'directory.'                    ],
  [ 'firmware-version',       '',            'Qualcomm Firmware version'     ],
  [ 'ar-yocto-path',          cpth(),        'Path to ar_yocto-cwe.tar.bz2 '
                                             'file.'                         ],
  [ 'enable-mangoh',          False,         'Enable mangOH layer.'          ],
  [ 'enable-qemu',            False,         'Enable QEMU build.'            ],
  [ 'cmdline-mode',           False,         'Do not execute build. Instead, '
                                             'launch a shell environment from '
                                             'which bitbake commands can be '
                                             'invoked, switched to the build '
                                             'directory.'                    ],
  [ 'debug-image',            False,         'Unsupported debug image. '
                                             'Use at your own risk.'         ],
  [ 'build-toolchain',        False,         'Build the toolchain image, '
                                             'and only the toolchain image.' ],
  [ 'recipe-args',            [str],
...
def enable_oe_layers(conf, ns):
  enable_layer_group(conf, ns.meta_oe_dir,
                           ['meta-oe', 'meta-networking',
                            'meta-python', pj('..', 'meta-gplv2')])
...
def enable_swi_layers(conf, ns):
  ...
  if enable_prop:
    layer_list += ['../meta-swi-extras/common', '../meta-swi-extras/meta-%s' % (mach)]

  # Proprietary SWI product layers
  if prod_fam != '':
    layer_list += [['../meta-swi-%s/common' % (prod_fam),
                 '../meta-swi-%s/meta-swi-%s' % (prod_fam, prod)]]

    if enable_prop:
      layer_list += [['../meta-swi-%s-extras/common' % (prod_fam),
                   '../meta-swi-%s-extras/meta-swi-%s-extras' % (prod_fam, prod)]]

      if enable_prop_src:
        layer_list += [['../meta-swi-%s-extras/meta-swi-%s-src' % (prod_fam, prod)]]
      else:
        layer_list += [['../meta-swi-%s-extras/meta-swi-%s-bin' % (prod_fam, prod)]]
  ...
  if enable_prop_src:
    ...
    layer_list += ['../meta-swi-extras/meta-swi-mdm9xxx-src',
                '../meta-swi-extras/meta-%s-src' % (mach)]

    # Product-specific source and no-arch layers
    if prod != '':
      layer_list += [['../meta-swi-extras/meta-%s-%s-src' % (mach, prod),
  ...
  if enable_prop_bin:
    layer_list += [['../meta-swi-extras/meta-swi-mdm9xxx-bin',
  ...
  def determine_yocto_mach(try_mach):
    ...
    mach_conf = find_file_in_layer_group('%s.conf' % (try_mach), meta_swi_dir, layer_list)
    if mach_conf:
  ...
  if prod != '':
    if ns.enable_recovery_image:
      yocto_mach = (determine_yocto_mach('%s-%s-rcy' % (mach, prod)) or
                    determine_yocto_mach('%s-rcy' % (mach)))
    else:
      yocto_mach = (determine_yocto_mach('%s-%s' % (mach, prod)) or
                    determine_yocto_mach(mach))
  else:
    ...
    if os.path.exists(local_dl):
      ...
      if indexof(res_conf, lambda ln: 'sierrawireless.local' in ln):
        ...
        if ns.enable_shared_sstate:
          sstate_mirror_url = 'http://get.legato/yocto/sstate/yocto-%s.$%s' % (yocto_major, yocto_minor)
          set_option(lconf, 'SSTATE_MIRRORS',
                     ['file://.*',
        ...
        set_option(lconf, 'PREMIRRORS_prepend',
    ...
    for ln in ima_conf:
      ...
      if var:
        ...
        if ('\\' in xval) or ('$' in xval) or ('"' in xval) or ("'" in xval):
          ...
          msg('%s: cannot fully expand syntax: %s -> %s' % (ima_conf_file, val, xval))
          sys.exit(1)
        os.environ[var] = xval
        if var in ima_vars:
...
def get_yocto_major_minor(ns):
  ...
  yocto_tag = proc_output(['git', '--git-dir=%s/.git' % (poky_dir),
                          'describe', '--tags', '--match=yocto-*'])
...
def tune_local_conf_icecc(lconf, ns):
  if ns.enable_icecc:
    if not get_option(lconf, 'ICECC_PARALLEL_MAKE'):
      ...
      set_option(lconf, 'ICECC_USER_PACKAGE_BL',
                 ' '.join(['ncurses e2fsprogs libx11 gmp',
...
def arg_parser_from_varspec(vs):
  ...
  parser.add_argument('--help', dest='help', default=False, const=True,
                      action='store_const', help="You're looking at it!")
  ...
  for s in vs:
    ...
    if isinstance(dfl, bool):
      parser.add_argument(oname, dest=nsname, default=dfl,
                          const=True, action='store_const', help=desc)
      parser.add_argument(noname, dest=nsname, default=dfl,
                          const=False, action='store_const')
    elif isinstance(dfl, str):
      parser.add_argument(oname, dest=nsname, default=dfl, metavar='<str>',
                          type=str, help=desc)
    elif isinstance(dfl, cpth):
      parser.add_argument(oname, dest=nsname, default=dfl.path, metavar='<path>',
                          type=str, help=desc)
      paths.append(nsname)
    elif isinstance(dfl, int):
      parser.add_argument(oname, dest=nsname, default=dfl, metavar='<int>',
                          type=flex_int, help=desc)
    elif isinstance(dfl, list):
      parser.add_argument(oname, dest=nsname, default=dfl[1:],
                          metavar=('<int>' if dfl[0] == int else '<str>'),
                          type=dfl[0], action='append', help=desc)
    else:
...
def find_variable(conf, var, after=None):
  ...
  idx = indexof(conf, lambda ln: ln.startswith(key1) or
                                 ln.startswith(key2) or
                                 ln.startswith(key3) or
                                 ln.startswith(key4))
