...
class Key:
    ...
    def __asn1_parse_openssl(self, parsed_file, arg_key_name):
        ...
        asn_parse_command = "openssl asn1parse -in {}.x509.pem -out {} > asn1parse.{}.log".format(
            arg_key_name, parsed_file, os.path.basename(arg_key_name)
        )
...
def process_file(image_type, json_file_path, arg_key_name, arg_unsigned_src, arg_signed_output, dp):
    ...
    if p.get("padding") is True:
        ...
        if image_type=="/boot":
            key.sign_image(image_path, "sig.bin", arg_key_name)
        else:
            pad_image_to_4k(image_path, "padded.bin", dp);
            key.sign_image("padded.bin", "sig.bin", arg_key_name)
            image_to_be_signed = "padded.bin"
    else:
        key.sign_image(image_path, "sig.bin", arg_key_name)

    with open("sig.bin", "rb") as sig:
        signature = sig.read()

    # Calculate get asn1 encoding
    dp.output("Encoding key {}.pk8 for ASN1".format(arg_key_name))
    key.parse_asn1_key("parsed.bin", arg_key_name)
    with open("parsed.bin", "rb") as parse:
            ...
            for arg_key_name in args.keyname:
                for json_file_path in args.file:
                    for image_type in args.type:
                        process_file(image_type, json_file_path, arg_key_name, arg_unsigned_src, arg_signed_output, dp)
