...
# It will be set to SWI_TRUE if UBI container signature is found on MTD
# partition containing Legato AF.
# If it's set to SWI_TRUE, but its Legato partition mount fails, system will
# reboot. Otherwise, if Legato mount fails system does not care.
IS_UBI_CONT=${SWI_FALSE}

# Initialize IS_UBI_CONT global variable.
# This method should be run before this variable is used, and should be called
# only once.
# Parameters:
...
# For secure version, cert chain is stored at one of the
# UBI volumes, So it should work with UBI file system
# Here secure mode and non-secure mode are supported.
# And the mode is determined by reading the efuse via the utility swi_auth
#
...
# ubix_3 -- root hash after signature (binary)
# ubix_4 -- cert chain (some of the keys, used for signature)
# The utility swi_auth will verify the signature of this image by reading the ubi volumes.
#
# If in non-secure mode, swi_auth will simple return SWI_SEC_NONE, and the upper layer
# will handle it by trying best efforts to mount it.
#
# Here is the usage for the utility swi_auth
# swi_auth <ubi_dev_num> [platform]
# ubi_dev_num - ubix
# platform - 9x40/9x28 or 9x15, default 9x40/9x28
#
# This utility will read efuse from specify register, if in non-secure mode stop and
# return SWI_SEC_NONE, If in secure mode, go on read data from ubix_2, ubix_3 and
# ubix_4 to do authentication.
#
...
check_security_auth()
{
    ...
    local mtd_dev_num=$2
    local ret_auth=
    local ubi_vol_total_num=
    local is_ubi_cont_local=${IS_UBI_CONT}
    ...
    if ! [ -x ${auth_cmd} ] ; then
     ...
     # If it is UBI image, try to attach it for later use no matter it is in secure or non secure mode.
     if [ ${is_ubi_cont_local} -eq ${SWI_TRUE} ] ; then
    ...
    # Return SWI_SEC_NONE if in non secure mode, no matter it is UBI image or not.
    # And the upper layer will handle it as dsecriped above.
    ${auth_cmd} ubi${ubi_dev_num}
    ret_auth=$?
    case "${ret_auth}" in
        4)
        ...
        6)
            ...
            echo "Secure version."
            if [ ${is_ubi_cont_local} -ne ${SWI_TRUE} ] ; then
                # ${is_ubi_cont_local} equal to FALSE means: There is something wrong with UBI partition.
                # But secure version should work with UBI, at this case we shall not continue.
...
set_boot_dev()
{
    ...
    # Initialize IS_UBI_CONT global variable for legato partition.
    is_ubi_cont_init ${mtd_dev_num}

    check_security_auth ${UBI_ROOTFS_DEVNUM} \
                        ${mtd_dev_num}
    secure=$?
    if [ ${secure} -eq ${SWI_AUTH_FAIL} ]; then
        #  If auth fail for secure version should not continue
        return ${SWI_ERR}
    fi

    if [ "$BOOTTYPE" != "yaffs2" ]; then

        if [ "${IS_UBI_CONT}" == "${SWI_TRUE}" ] ; then
